...
function onBeforeHandler(context) {
  ...
  return sandboxFromArguments(args).then(onProfile);
...
function sandboxFromArguments(args, options) {
  return new Bluebird(function(resolve, reject) {
    ...
    if (args.token) {
      if (args.profile && !options.allowProfile)
        return reject(
          new Cli.error.invalid(
            "--profile should not be specified with custom tokens"
          )
        );
    ...
    function loadProfile(profiles) {
      if (_.isEmpty(profiles)) {
        throw Cli.error.hint(
          "No webtask profiles found. To get started:\n" +
            Chalk.bold("$ wt init")
        );
      }
    ...
    function onBeforeRequest(request) {
      ...
      const result = proxy ? SuperagentProxy(request, proxy) : request;
