...
function printProfile(profile, options) {
    ...
    if (profile.openid) {
        ...
        var expired = (new Date() - new Date(profile.openid.valid_until)) < 0 ? '(still valid)' : '(expired)';
        console.log(Chalk.blue(Pad('Expires:', WIDTH)), Chalk.green(profile.openid.valid_until), Chalk.green(expired));
    }
    else {
