...
function awaitAvailable(profile, modules, options) {
    ...
    function validateAvailability(results) {
        ...
        if (options.onPoll) {
            try {
                options.onPoll(results, queued, failed);
            } catch (__) {
                // Discard exception
            }
        }
        ...
        if (failed) {
            return Bluebird.reject(Cli.error.serverError(`Webtask creation aborted because the module ${failed.name}@${failed.version} failed to build.`));
        }
