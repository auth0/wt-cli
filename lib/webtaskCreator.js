                ...
                .then(packageJsonBuffer => {
                    ...
                    return Bluebird.map(modules, Modules.resolveSpec)
                        .then(modules => {
                            ...
                            console.log(Chalk.bold('Verifying that the following modules from your package.json are available:'));
                            console.log();
                            ...
                            function onPoll(modules, queued, failed) {
                                if (lastResolve && lastResolve.length) {
                                    process.stdout.write(Ansi.cursorUp(lastResolve.length + 1) + Ansi.cursorLeft);
                                }

                                lastResolve = modules;

                                const status = modules.map(module => {
                                    const color = STATE_TO_COLOR[module.state] || Chalk.white;
                                    return `${Chalk.bold(`${module.name}@${module.version}`)}: ${color(module.state)}`;
                                });
                                ...
                                if (failed) {
                                    ...
                                    if (!spinnerInterval) {
                                        const renderSpinner = () => {
                                            const n = spinnerFrame++ % spinner.frames.length;
                                            const frame = spinner.frames[n];
                                            process.stdout.write(Ansi.cursorLeft);
                                            process.stdout.write(`${frame} waiting modules to be available`);
                                        };

                                        spinnerInterval = setInterval(renderSpinner, spinner.interval);
                                    ...
                                    process.stdout.write(Ansi.cursorPrevLine + Ansi.cursorLeft);
                                }
                            }
                        });
                }, _.constant(null));
