        ...
        if (args.packageJsonPath) {
            return Fs.readFileAsync(args.packageJsonPath)
                .then(packageJsonBuffer => {
                    ...
                    const modulesSpecs = _.map(packageJson.dependencies, (range, name) => ({ name, range }));
                    const resolvedModules = options.resolvedModules || {};

                    return Bluebird.map(modulesSpecs, (spec) => {
                        const resolved = resolvedModules[`${spec.name}@${spec.range}`];

                        return resolved || Modules.resolveSpec(spec);
                    })
                        .then(modules => {
                            ...
                            return available$
                                .tap((modules) => {
                                    args.meta['wt-node-dependencies'] = JSON.stringify(modules);
                                    args.meta['wt-node-dependency-specs'] = JSON.stringify(modulesSpecs);
                                })
                                .tap(() => {
                                    console.log();
