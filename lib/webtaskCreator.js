...
function createWebtaskCreator(args, options) {
    ...
    function createLocalWebtask(profile) {
        return args.protected
            ? createProtectedWebtask(profile)
            : createSimpleLocalWebtask(profile);
    }

    function createProtectedWebtask(profile) {
        var lastGeneration;

        // if apiSecret is empty, create one
        args.secrets.apiSecret = args.secrets.apiSecret  || require('crypto').randomBytes(64).toString('hex');

        var tmpFile = 'protected/tmp.js';
        console.log("Creating protected webtask from " + args.spec + " with name: " + args.name);
        return Fs.readFileAsync('protected/template.js', 'utf8')
            .then(function(template) {
                var relativeFileName = './' + path.relative(process.cwd() + '/protected', args.spec);
                return Fs.writeFileAsync(tmpFile, template.replace('{webtask}', relativeFileName));
            })
            .then(bundleProtectedWebtask);

        function bundleProtectedWebtask() {
    ...
    function createSimpleLocalWebtask(profile) {
        return args.bundle
