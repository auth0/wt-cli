    ...
    if (process.env.WT_DEBUG == 1)
        console.log('MIGRATE RESPONSE', e && e.message, r && r.statusCode, r && r.body);
                ...
                if (r.body.status === 'migrated') {
                    ...
                    return cb(new Error(r.body.message), r.body);
                }
                else {
...
exports.deleteNode4Assets = function (options) {
    ...
    return new Promise(function (resolve, reject) {
        return Async.series([
            (cb) => determineLegacyStack(cb),
            (cb) => deleteCronJobs(cb),
            (cb) => deleteWebtasks(cb)
        ], (e) => e ? reject(e) : resolve(null));
    });

    function determineLegacyStack(cb) {
        // This is to determine if the DNS has been repointed to the new stack already,
        // in which case no deletion should occurr.
        return Superagent.get('https://webtask.it.auth0.com/health')
            .end((e,r) => {
                profile.isLegacyNode4Stack = !e && r && r.body && r.body.stack_name && r.body.stack_name.indexOf('wa-usw-') === 0;
                return cb(e);
            });
    }

    function deleteCronJobs(cb) {
        if (!profile.isLegacyNode4Stack) return cb();
