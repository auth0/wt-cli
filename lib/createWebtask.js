...
function createWebtask(args, options) {
    ...
    function formatError(build) {
        build.stats.errors = build.stats.errors.map(err =>
            err.split('|').slice(0, -1).join('|'));

        if (args.watch) {
            const output = { generation: build.generation };

            _.forEach(build.stats.errors, function(error) {
                logger.error(output, 'Bundling failed: %s', error);
            });
    ...
    function formatOutput(build, url) {
        ...
        if (args.watch) {
            ...
            logger.info(
                Chalk.green('Webtask token %s') +
                    '\n\n%s\n\nYou can access your webtask at the following url:\n\n%s',
                options.action,
                Chalk.gray(build.webtask.token),
                Chalk.bold(url)
            );
        } else {
            logger.info(
                Chalk.green('Webtask %s') +
                    '\n\nYou can access your webtask at the following url:\n\n%s',
                options.action,
                Chalk.bold(url)
            );
