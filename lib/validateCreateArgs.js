...
function validateCreateArgs(args) {
    ...
    if (args.middleware.length) {
        ...
        const filter = mw => mw.startsWith('http') ? 'urls' : 'modules';
        const { urls, modules } = _.groupBy(args.middleware, filter);
        const middleware = [];
        ...
        args.meta['wt-middleware'] = _.union(middleware, urls).join(',');
    }
