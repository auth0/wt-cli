...
function validateCreateArgs(args) {
    ...
    if (args.profile.securityVersion === "v2" && args.secure) {
        throw Cli.error.invalid('The `wt create --secure` command is not supported by the target service security configuration.');
    }

    if (args['jwt-audience'] && !args['jwt-issuer'] || !args['jwt-audience'] && args['jwt-issuer']) {
        throw Cli.error.invalid(`Both 'jwt-audience' and 'jwt-issuer' are required or neither must be present.`);
    }
